    ...
    computed: {
        ...
        allCountries() {
            ...
            if (this.localizedCountries) {
                let countriesx = allCountries;
                let countries = countriesx.map(function(country) {
                    let localizedCountry = _this.localizedCountries.find(
                        function(localizedCountry) {
                            return localizedCountry.iso2 == country.iso2;
                        }
                    );
                    if (localizedCountry) {
    ...
    methods: {
        ...
        keyboardNav(e) {
            if (e.keyCode === 40) {
                ...
                if (~typedCountryI) {
                    this.selectedIndex =
                        this.preferredCountries.length + typedCountryI;
                    let selEle = this.$refs.list.children[this.selectedIndex];
                    if (
